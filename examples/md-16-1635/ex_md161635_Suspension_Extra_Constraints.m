%--------------------------------------------------------------------------
% ex_md161635_Suspension_Extra_Constraints.m
...
%--------------------------------------------------------------------------
function [pp,A,unusefulFlag] = ex_md161635_Suspension_Extra_Constraints(pp,A,unusefulFlag)
    %----------------------------------------------------------------------
    ...
    if unusefulFlag ~= 1 % only if the graph is currently feasible
        while 1
            ...
            J = find(pp.NSC.Vfull(row)==2,1,'first'); % find first connection with another 2-port component
            if isempty(J)
...
%                 pp.labels.N(keep) = str2double(['99',str]); % assign new number
                ...
                pp.labels.N(keep) = base2dec(upper(newlabel),36)'; % assign new number
